...
export function BuyTogether({ currentProduct, relatedProducts }: BuyTogetherProps) {
  ...
  const toggleProduct = (productId: string) => {
    ...
    if (productId === currentProduct.id) return; // Can't unselect current product
  ...
  const handleAddAll = () => {
    ...
    if (addedCount > 0) {
      toast({
        ...
        description: `${addedCount} produto(s) adicionado(s) ao carrinho.`,
      });
    } else {
      toast({
        title: 'Selecione os tamanhos',
        description: 'Adicione cada produto individualmente selecionando o tamanho.',
        variant: 'destructive',
      });
    }
  };
  ...
  return (
    <section className="py-8 bg-muted/30">
      <div className="container-custom">
        <h2 className="text-2xl font-bold text-center mb-8">Compre Junto</h2>
              ...
              return (
                <div key={product.id} className="flex items-center gap-4">
                  ...
                  >
                    ...
                    />
                    <p className="text-sm font-medium line-clamp-2 text-center max-w-[120px]">
                      {product.name}
                    </p>
